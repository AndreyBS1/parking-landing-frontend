...
import { approvePurchaseRequest } from '../api/approve-purchase-request'
import { getPurchaseRequests } from '../api/get-purchase-requests'
import { queryClient } from '../api/query-client'
import { rejectPurchaseRequest } from '../api/reject-purchase-request'
...
export default function PurchaseRequestsPage() {
  ...
  const { mutate: approvePurchaseRequestMutation, isPending: isApprovePending } =
    useMutation({
  ...
  const { mutate: rejectPurchaseRequestMutation, isPending: isRejectPending } =
    useMutation({
        ...
        {selectedPurchaseRequest?.status === PurchaseRequestStatusesEnum.Idle && (
          <Group grow>
            ...
            <Button
              ...
              classNames={{ root: 'bg-black' }}
              onClick={() =>
                rejectPurchaseRequestMutation(selectedPurchaseRequest?.id || 0)
              }
            >
              Отклонить
            </Button>
            <Button
              ...
              classNames={{ root: 'bg-black' }}
              onClick={() =>
                approvePurchaseRequestMutation(selectedPurchaseRequest?.id || 0)
              }
            >
              Одобрить
            </Button>
